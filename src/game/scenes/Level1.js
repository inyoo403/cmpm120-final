...
export default class Level1 extends BaseScene {
    ...
    createNextLevelPoint() {
        ...
        const point = layer.objects.find(obj => obj.name === '1-2');
        if (point) {
            const trigger = this.add.rectangle(point.x, point.y, point.width, point.height);
            this.physics.add.existing(trigger, true);
            this.physics.add.overlap(this.player, trigger, () => {
                const hasKey1 = this.registry.get('hasKey1');
                if (this.cursors.down.isDown && !this.isTransitioning && hasKey1) {
                    this.startLevelTransition();
                } else if (this.cursors.down.isDown && !hasKey1) {
                    // UI 씬을 통해 메시지 표시
                    const uiScene = this.scene.get('UI');
                    if (uiScene) {
                        uiScene.showCenterMessage('Need Key 1!');
                    }
                }
            });
        }
